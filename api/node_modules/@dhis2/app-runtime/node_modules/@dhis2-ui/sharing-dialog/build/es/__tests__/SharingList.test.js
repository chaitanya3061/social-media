...
import { SHARE_TARGET_EXTERNAL, SHARE_TARGET_PUBLIC, SHARE_TARGET_GROUP, SHARE_TARGET_USER, defaultSharingSettings } from '../sharing-constants.js';
...
describe('SharingDialog widget - SharingList component', () => {
  ...
  const getSharingListComponent = props => {
    if (!shallowSharingListComponent) {
      shallowSharingListComponent = shallow( /*#__PURE__*/React.createElement(SharingList, props));
    }
  ...
  it.skip('renders a SharingListItem for external access', () => {
    const external = getSharingListComponent(props).findWhere(n => n.prop('target') === SHARE_TARGET_EXTERNAL);
  ...
  it('renders a SharingListItem for public access', () => {
    const external = getSharingListComponent(props).findWhere(n => n.prop('target') === SHARE_TARGET_PUBLIC);
    expect(external).toHaveLength(1);
  ...
  it('renders a SharingListItem for a user group access', () => {
    ...
    const group = getSharingListComponent(props).findWhere(n => n.prop('target') === SHARE_TARGET_GROUP);
  ...
  it('renders a SharingListItem for a user access', () => {
    ...
    const user = getSharingListComponent(props).findWhere(n => n.prop('target') === SHARE_TARGET_USER);
