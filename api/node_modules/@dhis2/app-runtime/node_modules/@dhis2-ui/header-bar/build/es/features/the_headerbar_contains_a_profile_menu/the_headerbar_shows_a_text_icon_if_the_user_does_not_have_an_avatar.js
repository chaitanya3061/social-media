...
Then(`the headerbar contains a text icon of size 36px with the user's initials`, () => {
  cy.fixture('HeaderBar/me').then(({
    ...
    cy.get('[data-test="headerbar-profile-icon-text"]').should('be.visible').and('have.css', 'height', '36px').and('have.css', 'width', '36px').and('contain', getInitials(name));
  });
});
...
Then(`the profile menu contains a text icon of size 48px with the user's initials`, () => {
  cy.fixture('HeaderBar/me').then(({
    ...
    cy.get('[data-test="headerbar-profile-menu-icon-text"]').should('be.visible').and('have.css', 'height', '48px').and('have.css', 'width', '48px').and('contain', getInitials(name));
  });
});