...
(0, _steps.Given)('the highlighted item is not visible due to a set filter', () => {
  ...
  cy.get('@initiallyHighlighted').then($initiallyHighlighted => (0, _common.extractOptionFromElement)($initiallyHighlighted)).as('hiddenHighlighted');
...
(0, _steps.Then)('the option is visible', () => {
  cy.all(() => cy.get('@hiddenHighlighted'), () => cy.get('{transfer-sourceoptions} {transferoption}')).should(([hiddenHighlighted, $options]) => {
    const $hiddenHighlighted = $options.filter((index, optionEl) => {
      const option = (0, _common.extractOptionFromElement)(optionEl);
