import { CustomDataProvider } from '@dhis2/app-runtime';
import { storiesOf } from '@storybook/react';
import React from 'react';
import { OrganisationUnitTree } from '../index.js';
import { StatefulMultiSelectionWrapper, dataProviderData, namespace } from './common.js';
storiesOf(namespace, module).add('No initially expanded paths', () => /*#__PURE__*/React.createElement(CustomDataProvider, {
  data: dataProviderData
}, /*#__PURE__*/React.createElement(StatefulMultiSelectionWrapper, null, ({
  onChange
}) => /*#__PURE__*/React.createElement(OrganisationUnitTree, {
  roots: "A0000000000",
  onChange: onChange
})))).add('initially expanded paths', () => /*#__PURE__*/React.createElement(CustomDataProvider, {
  data: dataProviderData
}, /*#__PURE__*/React.createElement(StatefulMultiSelectionWrapper, null, ({
  onChange
}) => /*#__PURE__*/React.createElement(OrganisationUnitTree, {
  roots: "A0000000000",
  onChange: onChange,
  initiallyExpanded: ['/A0000000000']
}))));